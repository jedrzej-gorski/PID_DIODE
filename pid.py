...
class PID:
    ...
    def run_pid(self):
        ...
        p = figure()
        p.line(self.timestamp, self.input_intensity, line_width=2)
        save(p)
        output_file(filename="linia1.html", title="Wykres natężenia diody")
        r = figure()
        r.line(self.timestamp[0:1000], self.output_intensity, line_width=2)
        save(r)
        output_file(filename="linia2.html", title="Wykres natężenia środowiska")
        q = figure()
        q.line(self.timestamp, self.environment_intensity, line_width=2)
        save(q)
